...
class User(UserMixin):
    ...
    def firebase_custom_token(self):
        """Generates a Firebase custom token for the user."""
        try:
            # Generate custom token (bytes) and decode to string
            token = firebase_auth.create_custom_token(self.id)
            return token.decode('utf-8') if isinstance(token, bytes) else token
        except Exception as e:
            logger.error(f"Error generating firebase token for user {self.id}: {e}")
            return None
